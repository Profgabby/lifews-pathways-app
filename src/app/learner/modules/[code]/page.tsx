...
export default async function ModulePage({ params, searchParams }: { params: Promise<{ code: string }>; searchParams?: Promise<{ lang?: string }> }) {
  ...
  if (code.toUpperCase() !== "PF-01") notFound();
  const module = getPF01(language);
  const rtl = language === "ar";

  return <main className="learner-page" dir={rtl ? "rtl" : "ltr"} lang={language}>
    <header className="learner-header">
      <Link className="learner-brand" href={localizeHref("/pathways", language)}><img src="/lifews-mark.svg" alt="LIFEWS logo"/><div><strong>LIFEWS Pathways</strong><span>{module.level}</span></div></Link>
      <div className="learner-header-actions"><LanguageMenu currentLanguage={language}/><Link className="text-link" href={localizeHref("/learner", language)}>Learner space</Link></div>
    </header>
    ...
    <section><div className="eyebrow">Guided lessons</div><h2>{module.lessons.length} lessons with practical work, AI literacy and enterprise foundations</h2>
      <div style={{display:"grid",gap:18}}>{module.lessons.map((lesson,index)=><article className="card" key={lesson.id}>
        <div className="eyebrow">Lesson {index+1} · {lesson.id}</div><h2>{lesson.title}</h2><p><strong>Objective:</strong> {lesson.objective}</p>
        <h3>Learn</h3><ul>{lesson.teaching.map(x=><li key={x}>{x}</li>)}</ul>
        <p><strong>Activity:</strong> {lesson.activity}</p><p><strong>DIY / practical:</strong> {lesson.diy}</p><p><strong>AI:</strong> {lesson.aiLayer}</p><p><strong>Entrepreneurship:</strong> {lesson.enterprise}</p>
        <h3>Lesson quiz</h3>{lesson.quiz.map((q,qi)=><div key={q.question} style={{marginBottom:12}}><strong>{qi+1}. {q.question}</strong><ol type="A">{q.options.map(o=><li key={o}>{o}</li>)}</ol></div>)}
      </article>)}</div>
    </section>
