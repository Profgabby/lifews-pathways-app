...
export default async function KadaraPage({ searchParams }: PageProps) {
  ...
  const programs = localizeCurriculumPrograms(programsByArm("kadara"), language);
  ...
  return <div className="program-page-shell" dir={rtl ? "rtl" : "ltr"} lang={language}><header className="program-topbar"><Link href={localizeHref("/", language)} className="program-brand-link"><img src="/lifews-mark.svg" alt="LIFEWS"/><div><strong>LIFEWS GreenSkills</strong><span>{home.tagline}</span></div></Link><LanguageMenu currentLanguage={language}/></header><main>
    <section className="program-detail-hero kadara-hero"><div className="program-detail-number">02</div><div><div className="gs-eyebrow">{copy.eyebrow}</div><h1>LIFEWS Kadara</h1><h2>{copy.tagline}</h2><p>{copy.intro}</p><div className="program-detail-actions"><a href="#programs">{copy.primary}</a><Link href={localizeHref("/", language)}>{common.backToGreenSkills}</Link></div></div></section>
    ...
    <section className="fieldworks-callout"><div><div className="gs-eyebrow">{common.competenceNotAttendance}</div><h2>Assessment + FieldWorks + {common.skillsPassport}</h2><p>{assessmentStandard}</p><p>{evidenceStandard}</p><p>{deliveryStandard}</p></div><div className="fieldworks-metrics">{common.metrics.map(item=><span key={item}>{item}</span>)}</div></section>
  </main></div>;
}
